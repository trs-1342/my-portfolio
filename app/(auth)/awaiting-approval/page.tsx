...
export default function AwaitingApprovalPage() {
  ...
  /* Auth yüklendikten sonra kullanıcı yoksa login'e gönder */
  useEffect(() => {
    if (loading) return;
    if (!user) { router.push("/login"); return; }

    /* 10 saniyede bir onay durumunu kontrol et */
  ...
  return (
    <div style={{ width: "min(440px, 100%)" }}>
      <p
        className="mono"
        style={{ textAlign: "center", fontSize: "1.4rem", fontWeight: 700, color: "var(--accent)", marginBottom: "32px" }}
      >
        trs
      </p>
      ...
      <div className="glass" style={{ borderRadius: "20px", padding: "40px 32px", textAlign: "center" }}>
        ...
        <p style={{ fontSize: "0.85rem", color: "var(--text-2)", lineHeight: 1.7, marginBottom: "28px" }}>
          Kayıt işleminiz tamamlandı. Hesabınız yönetici tarafından onaylandığında
        ...
        <p className="mono" style={{ fontSize: "0.7rem", color: "var(--text-3)", marginTop: "20px" }}>
          Durum her 10 saniyede bir kontrol edilir.
        </p>
      </div>
    </div>
  );

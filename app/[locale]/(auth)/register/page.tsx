...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (msg === "auth/email-already-in-use") setError(t("errorExists"));
  ...
  return (
    <div style={{ width: "min(440px, 100%)" }}>
      ...
      <div className="glass" style={{ borderRadius: "20px", padding: "36px 32px" }}>
        ...
        <button onClick={handleGoogle} disabled={loading} style={{ width: "100%", padding: "11px", borderRadius: "10px", border: "1px solid var(--border)", background: "var(--bg-2)", color: "var(--text)", fontSize: "0.9rem", fontWeight: 500, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "10px", marginBottom: "20px", transition: "border-color 0.15s", fontFamily: "var(--font-sans)" }}>
        ...
        <form onSubmit={handleRegister} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          <AuthInput label={t("fullName")}      type="text"     value={displayName} onChange={setDisplayName} placeholder="Halil Hattab"         disabled={loading} />
          <AuthInput label={t("email")}         type="email"    value={email}       onChange={setEmail}       placeholder="ornek@mail.com"        disabled={loading} />
          <AuthInput label={t("password")}      type="password" value={password}    onChange={setPassword}    placeholder={t("passwordPlaceholder")} disabled={loading} />
          <AuthInput label={t("passwordRepeat")} type="password" value={password2}  onChange={setPassword2}   placeholder="••••••••"              disabled={loading} />
      ...
      <p style={{ textAlign: "center", marginTop: "20px", fontSize: "0.83rem", color: "var(--text-3)" }}>
        {t("hasAccount")}{" "}
        <Link href="/login" style={{ color: "var(--accent)", textDecoration: "none" }}>{t("loginLink")}</Link>
      </p>
    </div>
  );
}
